...
def create_agent(
    ...
    system_prompt = (
        ...
        "(e.g. amount per quantity), pass derivedMetrics: [{name, expression}] where expression "
        "uses metric refs like 'sales.amount/sales.quantity' — it is aggregated safely, no need "
        "for run_sql.\n"
        ...
        "- run_sql: read-only DuckDB SQL for custom filters, time buckets, and window functions "
        "that run_query cannot express. Prefer run_query + derivedMetrics for ratios/shares.\n"
